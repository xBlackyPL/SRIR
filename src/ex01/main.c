...
void child(int currentProcessRank) {
  ...
  MPI_Recv(message, RANGE, MPI_FLOAT, sourceProcessRank, 
      COMMUNICATION_TYPE, MPI_COMM_WORLD, &status);

  printf("[Child #%d]: Message from process of rank: %f \n", currentProcessRank, message[0]);

  free(message);

  printf("[Child #%d]: Communication time %lf\n", currentProcessRank, MPI_Wtime() - startTime);
}
...
void parent(int numberOfProcesses) {
  ...
  for(; sourceProcessRank < numberOfProcesses; ++sourceProcessRank, ++destinationProcessRank) {
    MPI_Recv(message, RANGE, MPI_FLOAT, sourceProcessRank, 
        COMMUNICATION_TYPE, MPI_COMM_WORLD, &status);
